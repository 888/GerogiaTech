...
static uint64_t find_free_frame(stats_t *stats) {
    ...
    /********* TODO ************/
    uint64_t lfu = current_pagetable[rlt[0].vpn].frequency; // least frequently used
    uint64_t num_rlt_e = 1 << rlt_size;
    for (int i = 0; i < num_rlt_e; i++) {
        if (rlt[i].valid == 0) {
            victim_pfn = i;
            i = num_rlt_e + 1;
        } else if (current_pagetable[rlt[i].vpn].frequency < lfu) {
            lfu = current_pagetable[rlt[i].vpn].frequency;
            victim_pfn = i;
        }
    }

    //deal with victim page table entry
    if (rlt[victim_pfn].valid == 1) {
