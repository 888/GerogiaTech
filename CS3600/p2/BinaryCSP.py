...
class CantAttackConstraint(BinaryConstraint):
    def isSatisfied(self, value1, value2):
    ...
    for var in domains:
        if not assignment.isAssigned(var):
            if nextVar is None:
                if not assignment.isAssigned(var):
                    nextVar = var
                    minLen = len(domains[nextVar])
            else:
                curLen = len(domains[var])
                if curLen < minLen:
                    nextVar = var
                    minLen = curLen
                    curConstraints = -1
                if curLen == minLen:
                    if curConstraints < 0:
                        curConstraints = getNumConstraints(nextVar,csp)
                    otherConstraints = getNumConstraints(var,csp)
                    if otherConstraints > curConstraints:
                        nextVar = var
                        minLen = curLen
                        curConstraints = otherConstraints
    return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    """Question 3"""
    constraining = getConstraining(assignment,csp,var)
    legalMoves = {}
    for val in values:
        legalMoves[val] = getLegalMoves(var,val,constraining,assignment,csp)
    values.sort(key=lambda val: -legalMoves[val])
    return values

def getLegalMoves(var,val,constraining,assignment,csp):
    legalMoves = 0
    assigned = assignment.assignedValues[var]
    assignment.assignedValues[var] = val
    for other in constraining:
        domain = csp.varDomains[other]
        for dom in domain:
            if consistent(assignment,csp,other,dom):
                legalMoves+=1
    assignment.assignedValues[var] = assigned
    return legalMoves
...
def getArcs(assignment,csp,var):
    arcs = deque()
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            otherVar = constraint.otherVariable(var)
            if assignment.assignedValues[otherVar] is None:
                arcs.appendleft((var,otherVar,constraint))
    return arcs
"""
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    """Question 5"""
    arcs = getArcs(assignment,csp,var)
    while len(arcs) > 0:
        arc = arcs.pop()
        inf = revise(assignment,csp,arc[0],arc[1],arc[2])
        if inf is None:
            for inference in inferences:
                assignment.varDomains[inference[0]].add(inference[1])
            return None
            ...
            if len(inf) is not 0:
                arcs.extendleft(getArcs(assignment,csp,arc[1]))
                inferences = inferences.union(inf)
    return inferences

def getArcsNoAssignmentCheck(assignment,csp,var):
    arcs = deque()
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            otherVar = constraint.otherVariable(var)
...
def AC3(assignment, csp):
    """Hint: implement revise first and use it as a helper function"""
    vars = csp.varDomains.keys()
    arcs = deque()
    for var in vars:
        arcs.extendleft(getArcsNoAssignmentCheck(assignment,csp,var))
    while len(arcs) > 0:
        arc = arcs.pop()
        inf = revise(assignment,csp,arc[0],arc[1],arc[2])
        if inf is None:
            ...
            if len(inf) is not 0:
                arcs.extendleft(getArcsNoAssignmentCheck(assignment,csp,arc[1]))
    return assignment
"""
